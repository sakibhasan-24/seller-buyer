...
export default function Edit() {
  ...
  useEffect(() => {
    if (newItems && newItems.userIdentify !== auth.currentUser.uid) {
      toast.error("you can't edit this page");
      navigate("/");
    }
  }, [newItems, auth.currentUser.uid, navigate]);
  useEffect(() => {
    setLoading(true);
    const fetchItems = async () => {
      ...
      if (docSnap.exists()) {
        setNewItems(docSnap.data());
        setFormData({ ...docSnap.data() });
        setLoading(false);
      } else {
        navigate("/");
        toast.error("not exists");
      }
    };
    fetchItems();
  }, []);
  ...
  const handleSubmitForm = async (e) => {
    ...
    const docRef = await updateDoc(doc(db, "listings", params.id), newFormData);
    // console.log(docRef);
    setLoading(false);
